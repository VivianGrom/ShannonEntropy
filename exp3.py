...
D	500 kyr	- 0.001 to 0.003   - 500 kyr - 0.003 to 0.0015
...
class Spatial_Autocorrelation:
    ...
    def plot_morans_i_and_gearys_c(self):
        ...
        ax1.plot(self.time_steps, moran_values, marker='o', color=color_moran, label="Moran's I")
        ax1.tick_params(axis='y', labelcolor=color_moran, labelsize=tick_font_size)
        ...
        ax2.plot(self.time_steps, geary_values, marker='s', linestyle='--', color=color_geary, label="Geary's C")
        ax2.tick_params(axis='y', labelcolor=color_geary, labelsize=tick_font_size)
...
uplift_gradient = np.linspace(0.0015, 0.001, n_colms)  # Reverse gradient from 1e-3 to 0
uplift_grid = np.tile(uplift_gradient, n_rows)  # Extend to all rows
...
uplift_gradient = np.linspace(0.002, 0.001, n_colms)  # Reverse gradient from 1e-3 to 0
uplift_grid = np.tile(uplift_gradient, n_rows)  # Extend to all rows
grid.at_node['uplift_rate'][:] = uplift_grid  # Update the grid field
...
ax2.errorbar(experiments, metrics["Soil"]["Moran's I Mean"], 
             ...
             fmt='o', color='blue', label="Moran's I", capsize=5)
...
ax2_2.errorbar(experiments, metrics["Soil"]["Geary's C Mean"], 
               ...
               fmt='o', color='red', label="Geary's C", capsize=5)

# Set labels and title for Soil
ax2.set_title("Soil Metrics across Experiment 3: Moran's I and Geary's C", fontsize=17)
ax2.set_ylabel("Moran's I", fontsize=18, color='blue')
ax2_2.set_ylabel("Geary's C", fontsize=18, color='red')
